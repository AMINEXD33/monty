...
*@stack: a pointer to a stack (see monty.h for more info)
*/
void main_interpreter(
    char *line_buffer,int buffer_lenght,unsigned int line_number,struct stack_s *stack)
{
    int x = 0;
    ...
    list = get_command_str_list(line_buffer, buffer_lenght, space_needed_val);
    /*map the command to its UPCODE function*/
    main_map(stack, line_number, list, space_needed_val);
...
char **get_command_str_list(
    char *line_buffer, int buffer_lenght, int *space_needed)
{
    int x = 0;
...
*/
void malloc_err1(char *buffer1, char *buffer2)
{
